...
namespace Wave.Essence.BundlePreview
{
	public class BundlePreviewManager
	{
		...
		private static AndroidArchitecture loaderAndroidArchitecture = AndroidArchitecture.ARMv7;
		...
		private static bool BuildAssetBundles(List<SceneData> sceneDataList)
		{
			...
			//Start building
			Dictionary<string, string> resourceAssetReferences = new Dictionary<string, string>();
		...
		private static bool PushAssetBundles(string loaderAPKPackageName, bool enableADBUtilitiesLog)
		{
			...
			if (adbUtilitiesInstance.PullFiles(remote_AssetBundlePathAbsolute, local_RemoteAssetBundlePullPathAbsolute, out output, out error) == ADBUtilities.adbNormalExitCode)
			{
				//Read remote manifest
				Debug.Log("Reading Remote manifest...");
				AssetBundle pulledBundle = AssetBundle.LoadFromFile(Path.Combine(Path.Combine(local_RemoteAssetBundlePullPathAbsolute, remote_AssetBundlePathRelative), remote_AssetBundlePathRelative));
		...
		public static void BuildandRunLoaderAPK(string loaderAPKPackageName)
		{
			...
			string loaderScenePath = Path.Combine(Directory.GetParent(Directory.GetParent(pathToManager).FullName).FullName, loaderScenePathRelative).Replace("\\", "/");
			Debug.Log("Loader Scene Path: " + loaderScenePath);
...
#if UNITY_2018_1_OR_NEWER
			BuildReport report = BuildPipeline.BuildPlayer(new string[] { loaderScenePath }, finalAPKBuildPath, BuildTarget.Android,
...
#else
			string error = BuildPipeline.BuildPlayer(new string[] { loaderScenePath }, finalAPKBuildPath, BuildTarget.Android,
				BuildOptions.Development | BuildOptions.AutoRunPlayer);
		...
		private static void BackupAndChangePlayerSettings()
		{
			...
			PlayerSettings.Android.targetArchitectures = loaderAndroidArchitecture;
